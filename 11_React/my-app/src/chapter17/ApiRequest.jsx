...
function ApiRequest(props) {
  ...
  const handleRequestById = (id) => {
    // josnplaceholder에서 제공하는 테스트용 API를 호출
    axios.get(`https://jsonplaceholder.typicode.com/photos/${id}`)
      .then((response) => {
        console.log(response);
        setData(response.data); // axios 라이브러리 JOSN -> 객체로 변환해 주는 작업을 자동으로 해줌
      })
  ...
  return (
    <>
      <div>
        ...
        <button onClick={() => handleRequestByIdAsync(500)}>불러오기</button> 
      </div>
      {data && (
        <>
...
// 요청: 서버에서 정해놓은 규격대로 맞춰 보내야 됨, 어디(URL 주소)로 어떤 방법(GET/POST 등)으로 보낼지 등
