...
// 서버 만드는 코드
const server = http.createServer((req, res) => { // 서버로 요청이 오면 이 콜백 함수가 실행됨 { 매개변수명 맘대로 작명해도 되지만 관례적으로 req,res 사용 }
  // req: 요청(request) 객체, res: 응답(response) 객체
  
  // 여기에 어떻게 응답할지 코드로 작성하면 됨
...
// 개발시에는 번거로우므로 추후 nodemon{(라이브러리)} 사용할 예정

// (참고) PORT(문 또는 구멍에 비유)
// 컴퓨터는 랜선을 꽂아 인터넷이 연결되면 IP주소를 통해 외부 컴퓨터들과 통신을 할 수 있게 설계됨
...
// { 원래 서버 파일은 하나만 사용, 포트 번호를 다르게 하면 또 다른 서버 만들 수 있다는 예시 }
