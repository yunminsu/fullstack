...
module.exports = () => {
  ...
  passport.serializeUser((user, done) => {
    ...
    // done(null, { id: user._id, username: user.username }); // { 여러값 저장 방법 }
  });
  ...
  passport.deserializeUser(async (id, done) => { // { id라는 매개변수명은 들어오는 값에 따라 작명하면 됨(여기선 위 코드의 done(null, user._id) <- id 값) user에 대한 여러 값이면 -> user 등 }
    console.log(id); // 세션에 저장한 내용

    try {
      const user = await db.collection('user').findOne({ _id: new ObjectId(id) });
      done(null, user); // req.user에 저장
      // 이제 아무데서나 req.user를 쓰면 현재 로그인한 사용자의 정보를 쓸 수 있음 { req.user.username 등등 값을 불러 올 수 있음 }
    } catch (err) {
      done(err);
    }
  });
...
// 4. 그안에 저장된 아이디로 데이터베이스에서 사용자 조회
